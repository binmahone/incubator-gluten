...
TEST_F(ParallelCompressionTest, ParallelWithSingleBuffer) {
  ...
  // Single buffer: numBuffers <= 1, so parallel won't activate even with pool.
  auto result = BlockPayload::fromBuffers(
...
TEST_F(ParallelCompressionTest, ParallelWithManyThreads) {
  ...
  auto pool8 = std::make_unique<CompressionThreadPool>(8);
  auto result = BlockPayload::fromBuffers(
      Payload::kCompressed, 500, std::move(buffers), &isValidity, pool_, codec_.get(), 8, pool8.get());
  ASSERT_TRUE(result.ok());
  auto payload = std::move(*result);
  ASSERT_NE(payload, nullptr);
}
